...
class PredictorLocal:
    def __init__(self, config_path, checkpoint_path, relative=False, adapt_movement_scale=False, device=None, enc_downscale=1, smooth_ratio=0.0):
        self.device = device or ('cuda' if torch.cuda.is_available() else 'cpu')
        self.relative = relative
        self.adapt_movement_scale = adapt_movement_scale
        self.smooth_ratio = max(0.0, min(1.0, float(smooth_ratio)))  # 0=off, higher=more smoothing
        self.start_frame = None
        self.start_frame_kp = None
        self.kp_driving_initial = None
        self._prev_kp_driving = None  # for temporal smoothing
    ...
    def reset_frames(self):
        self.kp_driving_initial = None
        self._prev_kp_driving = None

    @staticmethod
    def _smooth_kp(kp_new, kp_prev, alpha):
        """Blend new keypoints with previous for temporal smoothing. alpha = weight of previous (0=no smooth, 1=freeze)."""
        out = {}
        out['value'] = alpha * kp_prev['value'] + (1 - alpha) * kp_new['value']
        if 'jacobian' in kp_new and kp_new['jacobian'] is not None:
            out['jacobian'] = alpha * kp_prev['jacobian'] + (1 - alpha) * kp_new['jacobian']
        return out
    ...
    def predict(self, driving_frame):
        ...
        with torch.no_grad():
            ...
            # Temporal smoothing: blend with previous keypoints to reduce jitter
            if self.smooth_ratio > 0 and self._prev_kp_driving is not None:
                kp_driving = self._smooth_kp(kp_driving, self._prev_kp_driving, self.smooth_ratio)
            self._prev_kp_driving = {k: v.clone() for k, v in kp_driving.items()}
