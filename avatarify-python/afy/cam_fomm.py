# Pylint: cv2 (OpenCV) exposes members at runtime; static analysis reports false no-member (E1101).
...
# Allow Mac to run locally on CPU/MPS (no GPU server required). May be slow.
# if _platform == 'darwin':
...
if __name__ == "__main__":
    ...
    predictor_args = {
        ...
        'smooth_ratio': getattr(opt, 'smooth', 0.0),
    }
    if opt.is_worker:
    ...
    log('Window opened. Press X to calibrate and start the avatar. (If you do not see it, try Cmd+Tab or check behind other windows.)')
    ...
    find_keyframe = False
    is_calibrated = False
    input_smooth = getattr(opt, 'input_smooth', 0.0)
    frame_smoothed = None  # previous frame for input smoothing
    ...
    try:
        while True:
            ...
            # Optional: smooth camera input to reduce noise (blend with previous frame)
            if input_smooth > 0 and frame_smoothed is not None:
                frame = (input_smooth * frame_smoothed + (1 - input_smooth) * frame).astype(np.uint8)
